...
const {
  db: { getUserModel },
  env: { getGoogleClientID, getGoogleClientSecret, getGoogleCallbackURL },
  http: { createUserJWT },
  error: { errRequired }
...
const getToken = ({ passport }) => (req, res, next) => {
  ...
  passport.authenticate(provider, { session: false })(req, res, () => {
    ...
    const successURL = successURLTemplate.replace('{token}', token)
...
const getCode = ({ passport }) => (req, res, next) => {
  ...
  passport.authenticate(provider, {
    session: false,
    state: JSON.stringify({ successURLTemplate })
  })(req, res, next)
}
